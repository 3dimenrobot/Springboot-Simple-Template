<template>
  <div class="app-container">
    ...
    <el-dialog :visible.sync="dialog" :close-on-click-modal="false" :before-close="cancel" :title="isAdd ? '新增现状' : '编辑现状'" append-to-body width="570px">
      <el-form ref="form" :inline="true" :model="form" :rules="rules" size="small" label-width="76px">
        <el-form-item label="现状描述" prop="temperature">
          <el-input type="textarea" :rows="5" v-model="form.text"  style="width:420px;" @input="change($event)"/>
        </el-form-item>
        <br><br>
...
<script>
  import {readList,create,remove} from '@/api/corn/presentSituation'
  import {currentRouteArea} from '@/api/corn/area'

  import Pager from '@/api/Pager'

  export default {
    name: 'PresentSituation',
    mixins: [Pager],
    data() {
      return {
        entity:{
